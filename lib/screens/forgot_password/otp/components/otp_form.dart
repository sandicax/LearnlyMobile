...
class ForgotFormOTP extends StatefulWidget {
  const ForgotFormOTP({
    Key key,
  }) : super(key: key);

  @override
  _ForgotFormOTPState createState() => _ForgotFormOTPState();
}

class _ForgotFormOTPState extends State<ForgotFormOTP> {
  ...
  void verify() {
    valid = 0;
    var res =
        EmailAuth.validate(receiverMail: ForgotPass.email, userOTP: finalcode);
    if (res) {
      valid = 1;
    } else {
      valid = 0;
